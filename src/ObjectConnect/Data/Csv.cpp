...
[[nodiscard]] std::string MakeCsvError(const std::size_t line, const std::size_t column,
...
public:
    CsvSyntaxError(const std::size_t line, const std::size_t column,
                   const std::string& detail)
...
[[nodiscard]] std::vector<CsvRecord> ParseRecords(std::string_view text) {
    ...
    if (state == FieldState::Quoted) {
        throw CsvSyntaxError(quotedStartLine, quotedStartColumn,
